...
export default function FieldCheckin() {
  ...
  // TYPE-SAFE STATES FOR VERCEL COMPILATION
  ...
  useEffect(() => {
    ...
    getDocs(q).then((snap) => {
      if (!snap.empty) {
        ...
        setError(`Vessel vector [${voyagerId}] is not active in the registry.`);
      }
      setLoading(false);
    }).catch((err) => {
      console.error("Fetch vessel fault:", err);
      setError('Failed to securely link with the vessel routing grid.');
      setLoading(false);
    });
  }, [voyagerId]);
  ...
  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center font-mono text-xs text-slate-600 animate-pulse">VERIFYING TARGET VESSEL MATRIX...</div>;
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col items-center justify-center p-4 relative">
      <main className="w-full max-w-md bg-slate-900/40 backdrop-blur-xl border border-slate-900 rounded-2xl p-6 space-y-6 shadow-2xl relative z-10">
        ...
        <header className="text-center space-y-1.5">
          <h1 className="text-xl font-black tracking-widest text-slate-200 uppercase">VESSEL PORTAL</h1>
          <p className="text-[10px] font-mono text-emerald-500 uppercase tracking-widest">SECURE FIELD LINK // {voyagerId}</p>
        </header>
        ...
        {error ? (
          ...
          <div className="space-y-4 text-center font-mono uppercase">
            <div className="p-4 bg-emerald-950/20 border border-emerald-900/40 rounded-xl text-emerald-400 text-[11px] tracking-wide">
              🚀 TELEMETRY TRANSMISSION SUCCESSFUL // ROUTE RECALIBRATED
            </div>
          ...
          <form onSubmit={handleTransmitCheckin} className="space-y-4 font-mono text-xs text-slate-400">
            ...
            <div className="space-y-1">
              <label className="text-[9px] font-bold tracking-widest uppercase block text-slate-500">CAPTURE VERIFICATION ASSET (STATIONERY/PHOTO)</label>
              <input
                type="file"
                accept="image/*"
            ...
            <button
              ...
              disabled={submitting}
            >
              {submitting ? 'TRANSMITTING CODES...' : 'TRANSMIT VESSEL TELEMETRY'}
            </button>
          </form>
        )}
      </main>
