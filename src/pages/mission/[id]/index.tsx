...
// Dynamically imported components for Leaflet map operations
...
export default function Home() {
  ...
  // NEW SIMPLIFIED TRACKING MODAL STATES
  const [selectedVesselId, setSelectedVesselId] = useState<string | null>(null);
  const [vesselModalTimeline, setVesselModalTimeline] = useState<any[]>([]);

  // CUSTOM RED PIN CONFIGURATION
  ...
  // BUILD THE LIVE CHRONICLE TIMELINE FOR THE SIMPLIFIED POPUP MODAL
  useEffect(() => {
    if (!selectedVesselId || !activeVessels[selectedVesselId]) {
      setVesselModalTimeline([]);
      return;
    }

    const baseline = activeVessels[selectedVesselId];
    const vesselLogs = telemetryLogs
      .filter(log => log.voyagerId && log.voyagerId.toUpperCase() === selectedVesselId.toUpperCase())
      .sort((a, b) => (a.timestamp?.seconds || 0) - (b.timestamp?.seconds || 0));

    const buildTimeline: any[] = [];
    let lastLat = parseFloat(String(baseline.latitude));
    let lastLng = parseFloat(String(baseline.longitude));
    let lastTimeMs = new Date(baseline.launchDate).getTime();

    if (!isNaN(lastLat) && !isNaN(lastLng)) {
      buildTimeline.push({
        id: 'LAUNCH',
        handlerName: 'LAUNCH BASE',
        reportedLocation: `LAUNCH LOCATION: ${baseline.originCity}`,
        latitude: lastLat,
        longitude: lastLng,
        timestamp: lastTimeMs,
        isLaunchPad: true,
        displayActionContext: `${selectedVesselId} LAUNCHED`
      });
    }

    vesselLogs.forEach((log) => {
      if (!log.isLaunchPad) {
        const cLat = parseFloat(String(log.latitude));
        const cLng = parseFloat(String(log.longitude));
        const logTimeMs = log.timestamp?.toDate ? log.timestamp.toDate().getTime() : new Date(log.timestamp).getTime();

        buildTimeline.push({
          ...log,
          timestamp: logTimeMs,
          latitude: isNaN(cLat) ? lastLat : cLat,
          longitude: isNaN(cLng) ? lastLng : cLng
        });
    ...
    setVesselModalTimeline(buildTimeline);
  }, [selectedVesselId, activeVessels, telemetryLogs]);
  ...
  const modalPolylinePoints: [number, number][] = vesselModalTimeline
    .map(p => [parseFloat(String(p.latitude)), parseFloat(String(p.longitude))] as [number, number])
    .filter(coord => coord && !isNaN(coord[0]) && !isNaN(coord[1]));

  const modalMapCenter = modalPolylinePoints.length > 0 ? modalPolylinePoints[modalPolylinePoints.length - 1] : [37.0902, -95.7129];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col md:h-screen overflow-x-hidden relative">
      ...
      <header className="p-4 border-b border-slate-900 bg-slate-900/40 backdrop-blur shrink-0 z-40 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="font-mono text-xs uppercase tracking-wider flex items-center gap-4">
          {authLoading ? (
      ...
      {/* CORE HOME PORTAL VIEWPORT MAP & MATRIX */}
              ...
              <Marker key={pin.vesselId} position={[pin.lat, pin.lng]} icon={customRedIcon || undefined}>
                <Popup>
                  <div className="text-slate-900 font-mono text-xs font-bold p-1">
                    ...
                    <button 
                      onClick={() => setSelectedVesselId(pin.vesselId)}
                      className="text-blue-500 underline block mt-2 text-[11px] uppercase font-black bg-transparent border-0 cursor-pointer p-0 text-left"
                    >
                ...
                {fleetRegistryIds.map((id) => {
                  ...
                  return isDeployed ? (
                    <button 
                      ...
                      className="border-2 rounded-xl p-2.5 text-center transition-all flex flex-col items-center justify-center cursor-pointer bg-blue-950/80 border-blue-500 hover:bg-blue-900 shadow-md w-full"
                    >
                    ...
                    <button 
                      ...
                      className="bg-slate-900/40 border border-slate-800 border-dashed hover:border-emerald-500 hover:bg-emerald-950/20 rounded-xl p-3 text-center flex flex-col items-center justify-center transition-all cursor-pointer group w-full"
                    >
      ...
      {/* NEW SIMPLIFIED SINGLE-PAGE CHRONICLE TRACKING history LOG MODAL */}
      {selectedVesselId && (
        <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-md z-50 flex items-center justify-center p-3 md:p-6 font-mono select-none">
          <div className="w-full max-w-6xl bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden flex flex-col h-[90vh] md:h-[80vh] shadow-2xl relative">
            
            {/* MODAL HEADER SUBPANEL BUTTON ROW */}
            <header className="bg-slate-950 p-4 border-b border-slate-850 flex justify-between items-center shrink-0">
              <div>
                <h3 className="text-base md:text-lg font-black uppercase text-white tracking-widest">{selectedVesselId} TRACKING RECORD</h3>
                <p className="text-[10px] text-slate-400 uppercase font-bold mt-0.5">Expedition Ledger History & Path Polyline Map vectors</p>
              </div>
              <button 
                onClick={() => setSelectedVesselId(null)}
                className="bg-slate-900 text-slate-300 border border-slate-800 hover:bg-slate-850 font-black px-4 py-2 rounded-xl text-xs uppercase cursor-pointer"
              >
                [Close Track]
              </button>
            </header>

            {/* RESPONSIVE LAYOUT CONTEXT BLOCK SWITCHES */}
            <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
              
              {/* COMPONENT 1: MAP WINDOW FRAME - FULL HEIGHT ON DECK, TOP HALF ON MOBILE */}
              <div className="w-full md:w-3/5 h-[40vh] md:h-full relative border-b md:border-b-0 md:border-r border-slate-850 bg-slate-950 shrink-0 md:shrink">
                {modalPolylinePoints.length > 0 ? (
                  <MapContainer center={modalMapCenter} zoom={5} style={{ height: '100%', width: '100%', background: '#020617' }}>
                    <TileLayer url="https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png" />
                    {modalPolylinePoints.length > 1 && <Polyline positions={modalPolylinePoints} color="#2563eb" weight={3} dashArray="5, 8" />}
                    {vesselModalTimeline.map((pt) => {
                      const lat = parseFloat(String(pt.latitude));
                      const lng = parseFloat(String(pt.longitude));
                      if (isNaN(lat) || isNaN(lng)) return null;
                      return (
                        <Marker key={pt.id || pt.timestamp} position={[lat, lng]} icon={customRedIcon || undefined}>
                          <Popup>
                            <div className="font-mono text-xs p-1 text-slate-900 space-y-0.5">
                              <div className="font-black text-blue-600 block uppercase">✍️ {pt.handlerName}</div>
                              <div className="text-[10px] font-black text-slate-700 uppercase">📍 {pt.reportedLocation}</div>
                              <div className="text-[9px] text-slate-400 font-bold">{new Date(pt.timestamp).toLocaleString()}</div>
                            </div>
                          </Popup>
                        </Marker>
                      );
                  ...
                  <div className="h-full w-full flex items-center justify-center text-slate-500 text-xs uppercase text-center p-4">Parsing coordinate arrays...</div>
                )}
              </div>

              {/* COMPONENT 2: SCROLLING CHECKINS CHRONICLE LIST - SCROLLS BELOW MAP ON PHONES, RIGHT SIDE PANEL ON DESKTOP */}
              <div className="flex-1 overflow-y-auto p-4 md:p-5 bg-slate-950/20 space-y-3 font-sans">
                <h4 className="text-xs font-mono font-black uppercase text-slate-400 tracking-wider mb-2 border-b border-slate-900 pb-2">Logged Hand-offs (Newest to Oldest)</h4>
                {[...vesselModalTimeline].reverse().map((point) => {
                  return (
                    <div key={point.id || point.timestamp} className="bg-slate-900/40 border border-slate-900 rounded-xl p-3.5 space-y-2 font-mono text-xs">
                      <div className="flex justify-between items-start gap-2">
                        <span className="text-slate-300 font-bold">Holder: <span className="text-white font-black">{point.handlerName}</span></span>
                        <span className="bg-slate-950 px-2 py-0.5 rounded border border-slate-850 text-slate-300 text-[10px] uppercase font-bold truncate max-w-[150px]">{point.reportedLocation}</span>
                      </div>
                      {point.displayActionContext && (
                        <div className="text-[10px] text-blue-400 font-black uppercase bg-blue-950/20 border border-blue-900/20 px-2 py-0.5 rounded w-fit">
                          📌 {point.displayActionContext}
                        </div>
                      )}
                      {point.imageUrl && (
                        <img src={point.imageUrl} alt="Log Frame Attachment" className="w-full max-h-40 object-cover rounded-lg border border-slate-950 mt-1 mx-auto" />
                      )}
                      <div className="text-[9px] text-slate-500 text-right font-medium">{new Date(point.timestamp).toLocaleString()}</div>
                    </div>
                  );
                })}
              </div>
      ...
      {/* ENTRANCE SPLASH GATE INTERCEPTOR OVERLAY */}
